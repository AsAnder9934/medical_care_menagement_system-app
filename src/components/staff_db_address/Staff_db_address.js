...
function Staff_db_address() {
  ...
  useEffect(() => {
    axios
      .get(
        `http://127.0.0.1:8080/geoserver/Hospital/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=Hospital%3Ahospitals_staff&maxFeatures=50&outputFormat=application%2Fjson&cql_filter=pesel='${pesel}'`
      )
      .then((response) => {
    ...
    <div className="patient_db_contact">
      <div className="patient_db_contact_top">
        <div className="patient_db_contact_name">
          ...
          {isEditing ? (
            <input
              type="text"
              name="name"
              value={`${patientData.name} ${patientData.surname}`}
        ...
        <div className="patient_db_contact_menu">
          <Link to={`/log_in/services/staff/db/contact/${patientData.pesel}`}>
...
export default Staff_db_address;
